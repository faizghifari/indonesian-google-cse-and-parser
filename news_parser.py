...
class NewsContentParser:
    ...
    def __normalize_content(self, text):
        regex = re.split("\n|\r", text)
        term = " ".join(regex)
        term = re.sub(r"\ +", " ", term)
    ...
    def __parse_content(self, content):
        ...
        for string in content.stripped_strings:
            if (("{" not in string and "}" not in string) and (string != "")) and (
                string is not None and "Baca " not in string
            ):
    ...
    def parse_content(self):
        ...
        if content is not None:
            ...
            parsed_content = self.__normalize_content(parsed_content)
            parsed_content = re.sub(r"\[ .{1,3} \]", "", parsed_content)
            parsed_sentences = self.__parse_sentences(parsed_content)
            return self.__make_response(True, parsed_content, parsed_sentences)